...
{
public:
    AnalyzerVisitor(logger::ILogger& logger, Context& context, Scope& scope, SymbolTable& symbolTable)
            :
            logger_(&logger),
            context_(&context),
            scope_(&scope),
    ...
    asg::Node operator()(const boost::variant<boost::recursive_wrapper<ast::InitlistNode>, boost::recursive_wrapper<ast::ExprNode>>& node)
    {
